  ...
  return (
    <div style={{ display:'flex', background:'#f8fafc', minHeight:'100vh' }}>
      ...
      <div className="main-content" style={{ marginLeft:'220px', flex:1, padding:'24px', minWidth:0, overflowX:'hidden' }}>
        <h1 style={{ margin:'0 0 4px', fontSize:'clamp(18px, 3vw, 24px)', fontWeight:'700', color:'#0f172a' }}>Dashboard</h1>
        <p style={{ margin:'0 0 24px', color:'#64748b', fontSize:'14px' }}>Welcome back, {localStorage.getItem('full_name')} 👋</p>

        <div className="stats-grid" style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(180px, 1fr))', gap:'14px', marginBottom:'24px' }}>
              ...
              <tbody>
                {recentAttempts.map(a => (
                  <tr key={a.id} style={{ borderTop:'1px solid #f1f5f9' }}>
                    ...
                    <td style={{ padding:'12px 16px', fontSize:'14px', color:'#475569', whiteSpace:'nowrap' }}>{a.exam_title}</td>
                    <td style={{ padding:'12px 16px', fontSize:'14px', color:'#475569' }}>{a.score}/{a.total}</td>
